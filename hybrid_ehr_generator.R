...
generate_hybrid_ehr_dataset <- function(
  ...
  patients_core <- tibble::tibble(
    ...
    mrn = sprintf("MRN%08d", sample(10000000:99999999, n_patients)),
    ...
    # Smoking status: mutually exclusive
    ...
    # COVID vaccination
    covid_vaccinated = if (covid_focused) {
      sample(
        c("Yes", "No", "Unknown"),
        n_patients,
    ...
    vaccine_doses = ifelse(
      ...
      sample(1:4, n_patients,
             prob = c(0.10, 0.15, 0.60, 0.15),
             replace = TRUE),
      0L
    ),
    ...
    address = replicate(
      ...
      paste(
        sample(100:9999, 1),
        sample(c("Main", "Oak", "Maple", "Cedar"), 1),
        sample(c("St", "Ave", "Rd"), 1)
      )
    ),
    city = sample(c("New York", "Los Angeles", "Chicago"),
    ...
    phone = paste0("(", sample(200:999, n_patients), ") ",
                   sample(100:999, n_patients), "-",
                   sample(1000:9999, n_patients)),
  ...
  if (covid_focused) {
    latest_encounters <- encounters %>%
      ...
      dplyr::filter(admission_datetime == max(admission_datetime)) %>%
      dplyr::ungroup() %>%
      dplyr::select(
        patient_id, encounter_id, symptom_to_ct_days, admission_to_ct_hours,
        o2_support_level, ward_type, on_systemic_steroids, on_anticoagulation,
        covid_test_result, covid_severity
      )

    latest_vitals <- vitals %>%
      dplyr::inner_join(
        latest_encounters %>% dplyr::select(patient_id, encounter_id),
        by = "encounter_id"
      ) %>%
      dplyr::group_by(patient_id) %>%
      dplyr::filter(measurement_datetime == max(measurement_datetime)) %>%
      dplyr::ungroup() %>%
      dplyr::select(
    ...
    latest_labs <- labs %>%
      ...
      dplyr::filter(collection_datetime == max(collection_datetime)) %>%
      dplyr::ungroup() %>%
    ...
    research_view <- patients_core %>%
      dplyr::select(
        patient_id, age_years, sex_male, bmi,
        smoker_current, smoker_former, smoker_never,
        dplyr::starts_with("hx_"), site_id_numeric
      ) %>%
      dplyr::left_join(latest_encounters, by = "patient_id") %>%
      dplyr::left_join(latest_vitals,     by = "patient_id") %>%
